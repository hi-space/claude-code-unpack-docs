...
소스 구현 (Codename 시스템 기반)은 도구 스키마를 **이름순으로 알파벳순 정렬**하여 유니코드 인식 문자열 비교를 사용합니다. 이는 도구가 내부적으로 어떻게 등록되거나 메모리에 저장되는 순서와 관계없이 순서가 결정적임을 보장합니다.

도구 컬렉션에서 모든 도구를 검색하고, 지연된 도구를 필터링하고, 로케일 인식 비교(`localeCompare()`)를 사용하여 도구 이름으로 정렬한 다음 각 도구를 이름, 설명, 매개변수 및 사용자 정의 필드를 포함한 스키마 표현으로 변환합니다.
...
~20K tokens이 캐시된 접두사에 있을 때:

| 가격 등급 | 캐시되지 않은 비용 | 캐시된 비용 | 요청당 절감액 |
|--------------|--------------|------------|---------------------|
...
수백 개의 요청이 있는 일반적인 세션 동안:

```
세션 매개변수:
- 세션당 200 API 요청 (일반적인 사용)
- 요청당 ~20K 캐시된 토큰
- Claude 3.5 Sonnet 가격 ($0.003 per 1K input tokens)
...
세션당 절감액: $10.20 (85% cost reduction)
```

규모 적용 (매월 10,000 세션):
- **월간 비용 감소: $102,000**

이러한 경제적 동기가 전체 코드베이스가 캐시 안정성 유지 주위에 구조화된 이유를 설명합니다.

### Cache 효율성 메트릭

시스템은 각 API 요청 후 캐시 성능 메트릭을 로깅합니다. 이 메트릭은 모니터링 및 최적화를 위한 대시보드로 흘러갑니다:

**요청별로 기록된 메트릭**:
- **Cache hit**: Prefix가 캐시에서 제공되었는지 (재처리되지 않음) 나타내는 부울값
- **Tokens processed**: API에 보낸 총 입력 토큰 수
- **Tokens from cache**: 캐시에서 제공된 토큰 수 (사용 기준으로 청구되지 않음)
- **Input tokens**: 실제로 청구된 토큰 (캐시된 토큰 제외)
- **Cache creation tokens**: 캐시 경계를 *생성*하는 데 사용된 토큰 (첫 캐시 쓰기에서만)

**집계된 세션 메트릭**:
- **Hit rate**: `(cacheHits / totalRequests)`. 일반적으로 세션 내 92-98%
- **요청당 평균 캐시된 토큰**: 총 20-25K prefix 중 18-20K
- **요청당 비용**: Claude 3.5 Sonnet의 경우 약 $0.06 (캐싱 없이 약 $0.60)
- **세션 절감**: 세션당 약 85% 비용 감소

**프로덕션 수준 메트릭** (API 사용량의 cache_read_input_tokens에서 계산):
- 월 10,000 세션 기준: 월간 $100K+ 절감
- 세션 내 일관된 92%+ 히트율 (cache breaks는 드문 일)
- 캐시 경계 오버헤드: prefix 토큰의 약 1-2% (첫 쓰기에서 cache_creation_input_tokens)

이 메트릭은 캐싱 아키텍처의 경제적 효율성을 검증하고 cache break 감지 및 prefix 안정성 메커니즘의 복잡성을 정당화합니다.
...
시스템 프롬프트 조립은 다음 단계를 따릅니다:

- **Prefix assembly**: 접두사 구성
- **Cache boundary insertion**: 캐시 경계 마킹
- **Tool ordering**: 도구 알파벳순 정렬
- **Suffix assembly**: 접미사 구성
