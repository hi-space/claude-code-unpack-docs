...
시스템 프롬프트는 행동 위험을 평가하기 위한 정교한 프레임워크를 포함합니다. 이것은 단순한 지침이 아닙니다. **PermissionChecker의 Layer 3 분류자 로직과 직접 매핑됩니다** ([권한 모델](../security/permission-model.md) 참조).
...
### PermissionChecker 분류로 매핑하는 방법

PermissionChecker (권한 모델의 Layer 3)는 같은 위험 차원을 통해 도구 호출을 평가합니다. 분류자는 도구 호출을 검사하고 파괴성 여부를 확인합니다 (파일 삭제, 데이터베이스 테이블 삭제, git 리셋). 파괴적이고 되돌릴 수 없으면 HIGH_RISK를 반환합니다. 도구 호출이 공유 상태에 영향을 미치는 가시적 작업인지 확인합니다 (git push, PR 생성, AWS 배포). 그렇다면 MEDIUM_RISK를 반환합니다. 그 외는 LOW_RISK를 반환하여 진행하도록 합니다.

교차 참조: [권한 모델 - Layer 3 분류자](../security/permission-model.md#layer-3-security-classifier)

## 안전 규칙 및 권한 모델 상호작용

Claude Code는 안전하지 않은 작업에 대항하는 **2계층 방어**를 구현합니다:

### Layer 1: 행동 제약 (시스템 프롬프트)
...
### Layer 2: 기술적 실행 (권한 모델)
PermissionChecker는 **도구 시스템이 실제로 허용하는 것**을 실행합니다:
- Layer 1 (고정 허용 목록): 읽기 전용 도구 자동 허용
- Layer 2 (사용자 규칙): 사용자 구성 패턴 자동 허용
- Layer 3 (분류자): Sonnet 4.6은 위험한 호출을 독립적으로 평가합니다
...
flowchart LR
    ...
    PermissionChecker["권한 모델:<br/>Layer 3 분류자"]
    User["사용자<br/>확인"]
...
4. **권한 모델** Layer 1/2/3이 `rm -rf` 호출 평가
...
sequenceDiagram
    ...
    participant Classifier as 분류자<br/>Layer 3
