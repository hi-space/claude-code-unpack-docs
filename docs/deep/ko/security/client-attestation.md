...
Bun의 아키텍처는 JavaScript 런타임 중에서 유일하다 (핵심 통찰: JavaScript 아래 Zig):

```
┌─────────────────────────────────────────┐
│  JavaScript/TypeScript 애플리케이션 코드 │  ← 검사, 패칭, 디버깅 가능
│─────────────────────────────────────────│
│  Bun JavaScript 엔진 (Zig)              │  ← 컴파일된 네이티브 코드
│─────────────────────────────────────────│
│  Bun HTTP 클라이언트 (Zig)              │  ← 해시 계산이 여기서 일어남
│  - TLS 구현                             │
│  - HTTP/2 멀티플렉싱                    │
│  - 요청 직렬화                          │
│  - ** 클라이언트 인증 해시 **           │
...
핵심 통찰: **JavaScript 몽키패칭은 Zig 계층에 도달할 수 없다**. 공통 우회 기법들이 실패한다:
...
Zig HTTP 전송 계층은 JavaScript 런타임 아래 완전히 작동하며 실제 암호화 인증 계산을 수행한다. Bun의 네이티브 HTTP 스택이 요청을 전송할 준비를 할 때, 플레이스홀더 값을 포함하는 모든 `cch` 헤더를 인터셉트하여 계산된 암호화 해시로 교체한다. 해시는 여러 입력에서 파생된다: 완전한 요청 본문 바이트, JavaScript 메모리에 절대 나타나지 않는 컴파일된 비밀 키, 그리고 재생 공격을 방지하는 타임스탬프.

계산은 HMAC-SHA256을 핵심 암호화 원시 데이터로 사용하여, 정확한 비밀 키가 Zig 계층에 컴파일된 정품 Claude Code 바이너리만 유효한 인증 해시를 생성할 수 있음을 보장한다. 최종 해시는 처음 16 바이트로 자르고 16진수로 포맷되어 플레이스홀더를 교체한다. 이는 JavaScript SDK가 요청을 핸드오프한 후에 일어나지만 네트워크 계층에 도달하기 전에, 실제 해시 계산이 JavaScript 디버거, 프록시, 인스트루먼테이션 도구에서 완전히 숨겨지도록 보장한다. 타임스탬프 포함은 공격자가 유효한 해시를 캡처하여 다른 요청에 대해 재사용할 수 있는 재생 공격으로부터 추가 보호한다.
