...
QueryEngine(Claude Code의 통합 실행 엔진)이 async 컨텍스트 내에서 실행되며, 팀원의 특정 프롬프트, 허용된 도구, 권한 설정을 받습니다. 이는 같은 Node.js 프로세스에서 여러 팀원이 동시에 실행되면서 도구 호출이 서로 간섭하지 않도록 합니다. 진행 상황은 하트비트와 상태 업데이트를 통해 추적되어, Coordinator가 정체된 작업을 감지할 수 있고 UI가 실시간 상태를 표시할 수 있습니다.

팀원이 완료되거나 실패하면, 그 결과는 AppState에 저장되고 Coordinator에 알려져서, 작업을 재시도하거나 다른 Agent 유형으로 에스컬레이션할지 결정할 수 있습니다.
...
**Mailbox 구조:**
Mailbox 파일은 taskId, status(pending, processing, completed, failed), task(prompt, tools, context), result(output, files, exitCode), 그리고 타임스탐프를 포함합니다.
...
Coordinator 프롬프트에는 여러 핵심 섹션이 포함됩니다:

Coordinator 프롬프트는 작업 분해, 작업자 할당, 품질 관리, 진행 상황 모니터링, 그리고 결과 집계를 포함합니다. 작업을 독립적인 하위 작업으로 분해하고, 작업의 특성에 따라 적절한 Agent 유형을 선택합니다. 품질 관리는 약한 작업을 거부하고 수정을 요청하며, TaskList를 통해 모든 작업자를 모니터링합니다. 모든 작업자가 완료되면 변경사항을 병합하고 통합된 솔루션을 검증합니다.

### Quality Assurance Directives

Coordinator 프롬프트는 각 Worker 출력을 비판적으로 평가하고, 표준 이하의 작업을 거부하며, 작업자 출력 간의 일관성을 보장하고, 조립된 결과가 원래 요구사항을 충족하는지 검증하도록 명시적으로 지시합니다.
...
Coordinator가 Agent 도구를 통해 Worker를 생성하면, 작업은 메타데이터와 함께 AppState에 들어갑니다: 작업 ID, Agent ID, 상태 및 타임스탐프. 작업은 **pending** (대기열에 있지만 시작되지 않음), **running** (활발하게 실행 중), **completed** (성공적으로 완료) 또는 **failed** (오류로 중지됨)의 단계를 통해 전환됩니다.

각 작업은 하트비트 메커니즘을 포함합니다. Agent는 정기적으로 진행 상황을 보고하여 `lastHeartbeat`를 업데이트합니다. 작업이 실행 중이지만 설정된 시간 초과 (일반적으로 5분) 동안 하트비트가 도착하지 않으면, Coordinator는 이를 정체됨으로 표시하고 더 오래 기다리거나 UI에 중재 신호를 보낼 수 있습니다.

CoordinatorAgentStatus React 컴포넌트는 AppState 변경을 구독하고 실시간으로 모든 활성 작업을 표시하여 상태 배지, 진행 표시기, 오류 메시지 및 정체된 작업 경고를 보여줍니다. 이를 통해 사용자는 폴링 없이 병렬 Worker를 모니터링할 수 있으며, Coordinator가 어떤 작업이 진행 중이고 어떤 작업이 차단되었는지 가시성을 확보할 수 있습니다.

작업 결과(최종 메시지, 파일 목록, 종료 코드)는 작업 기록과 함께 AppState에 저장되어 Coordinator가 합성에 접근할 수 있고 UI가 표시할 수 있습니다.
...
Worker는 [Agent 도구](../tools/agent-tools.md)를 사용하여 하위 Agent로 생성됩니다. 각 Worker는:

- 전체 작업 설명을 수신합니다 (공유 컨텍스트 없음)
- 자신의 Context Window를 가지고 독립적으로 작동합니다
- 사용 가능한 모든 도구를 사용할 수 있습니다
...
Coordinator 모드는 두 가지 메커니즘으로 제어됩니다. Bun 컴파일 타임 Feature Flag (`COORDINATOR_MODE`)와 환경 변수 (`CLAUDE_CODE_COORDINATOR_MODE`)가 함께 작동합니다. Feature Flag는 Coordinator 코드가 모두 번들에 포함되는지 여부를 결정합니다. 환경 변수는 Feature Flag가 활성화되어 있을 때 런타임 활성화를 제어합니다.

이 설계는 Feature Flag를 통해 공개 빌드에서 Coordinator 코드 번들링을 방지하는 동시에 (데드 코드 제거 통해), 내부 빌드가 환경 변수를 통해 런타임에 활성화/비활성화할 수 있게 합니다.
...
Query Engine 수준에서 Coordinator 모드 활성화가 발생합니다. 새로운 Agent 요청이 도착할 때 시스템은 컴파일 시 Feature Flag가 활성화되어 있는지, 런타임에 환경 변수가 활성화되도록 설정되어 있는지, 그리고 사용자의 프롬프트가 병렬 작업을 시사하는지 (휴리스틱 키워드 "multi-file", "multiple", "parallel", "components") 확인합니다.

모든 확인이 통과하면 Query Engine이 독립 실행형 Agent 대신 Coordinator를 생성할 수 있습니다. Coordinator는 전체 사용자 프롬프트를 받고 이를 하위 작업으로 분해하여 독립적 작업에 대한 Worker를 시작합니다.
