...
**QueryEngine 격리**: 각 Subagent는 자신의 QueryEngine 인스턴스를 받으며, 격리된 대화 히스토리, 독립적인 도구 상태 관리, 별도의 작업 디렉토리 컨텍스트, Agent 간 데이터 유출 없음을 보장합니다.

**유형별 시스템 프롬프트**: Subagent 프롬프트는 유형별 지침, Agent별 제약 및 기능, 부모 Agent 추적 정보, 실행 경계 (읽기 전용, 구현 전용 등)를 포함하도록 구성됩니다.

**초기화 시 도구 레지스트리**: 도구는 Subagent 생성 시점에 필터링되며, 런타임 중이 아닙니다. 이는 도구 제한 우회를 방지하고 자식이 허용된 도구 레지스트리만 받도록 보장합니다.

**부모-자식 계층 구조**: 부모 Agent 식별자를 통해 추적되며, 계층적 중단 신호 전파를 활성화하고, 부모가 자식 진행 상황을 모니터링하고, 중첩된 Subagent 생성 (Agent → Subagent → Sub-subagent)을 지원합니다.
...
| **claude-code-guide** | Edit, Write, Bash, Agent, NotebookEdit, ExitPlanMode | Glob, Grep, Read, WebFetch, WebSearch | Claude Code 사용 질문, 설정 가이드 |
...
**Agent Tool**: 서브에이전트가 자신의 서브에이전트를 생성하는 것을 방지합니다 (general-purpose 제외). 이는 폭발적인 생성 계층 구조를 방지하고 컨텍스트 관리를 단순화합니다.

**파일 수정 도구 (Edit, Write, NotebookEdit)**: 읽기 전용 에이전트 (Explore, Plan, verification)가 코드를 수정하는 것을 방지합니다. 이 유형들은 분석, 계획, 검증을 위해 설계되었으며, 구현을 위함이 아닙니다.

**계획 모드 제어**: Plan 에이전트에만 관련되며, 의도하지 않은 모드 종료를 방지합니다.
...
Node.js `AsyncLocalStorage`는 async 실행 체인을 통해 데이터가 흐르도록 하는 네임스페이스를 제공하며, 명시적 매개변수 전달이 필요 없습니다. 각 에이전트는 고유한 AsyncLocalStorage 인스턴스를 얻어 ID, 권한, 중단 신호와 같은 컨텍스트를 저장합니다.

**작동 방식:**

1. **컨텍스트 생성** - 에이전트가 생성되면 다음을 포함하는 새로운 컨텍스트 객체가 생성됩니다:
   - `agentId`: 에이전트의 고유 식별자
   - `agentType`: Agent 도구를 통해 생성된 경우 'subagent', swarm 팀원인 경우 'teammate'
   - Subagent의 경우: 서브에이전트 이름 (예: "Explore", "Plan"), 부모 세션 ID, 권한
   - 팀원의 경우: Agent 이름, 팀 이름, 시각적 식별자, 모드 요구사항
   - `abortController`: 계층적 취소용

2. **컨텍스트 바인딩** - 에이전트의 전체 실행은 async 컨텍스트 내에서 실행되며, 모든 중첩된 작업 (도구 호출, 타이머, async 함수)에 자동으로 전파됩니다.

3. **컨텍스트 검색** - 도구 호출, 중첩 함수, 타이머는 명시적 매개변수 전달 없이 컨텍스트에 접근할 수 있습니다.

4. **격리 보장** - 여러 에이전트가 동시에 실행될 때(백그라운드 에이전트, 병렬 탐색), 각 에이전트는 자신의 격리된 컨텍스트 네임스페이스를 유지합니다. Agent A의 이벤트는 절대 Agent B의 컨텍스트를 보지 않습니다.
...
부모 Agent가 자식을 생성하면, 부모의 abort 신호를 자식에게 전달합니다. 부모가 취소되면, 신호는 계층을 통해 자식에 자동으로 전파되어 우아한 정리를 트리거합니다.

### Cleanup 보장

Abort 신호를 받으면:
1. 모든 대기 중인 도구 호출이 취소됩니다
2. Worktree 정리가 시작됩니다 (해당되는 경우)
3. 파일 핸들이 닫힙니다
4. QueryEngine 인스턴스가 소멸됩니다
5. 결과는 `{ aborted: true }` 상태로 반환됩니다
...
**Spawning (0-100ms)**: Agent 유형 설정을 로드하고, 도구 레지스트리를 필터링하고, 시스템 프롬프트를 구성합니다.

**Initialized (100-200ms)**: QueryEngine 인스턴스를 생성하고, AsyncLocalStorage 컨텍스트를 설정하고, AbortController 체인을 구성하고, 격리가 요청된 경우 Worktree를 생성합니다.

**Running**: Agent가 메시지를 처리하고, QueryEngine 내에서 도구 호출을 실행하고, 상태를 AsyncLocalStorage에 유지하고, 유형이 허용하는 경우 자식 Agent를 생성할 수 있습니다.

**Completed/Failed/Aborted**: 결과 객체를 준비하고, 해당 경우 오류 세부 정보를 캡처하고, 자식에 abort 신호를 전파합니다.

**Cleanup**: 파일 핸들을 닫고, 해당되는 경우 Worktree를 제거하고, AsyncLocalStorage 컨텍스트를 파괴하고, QueryEngine을 할당 해제합니다.

### Spawn 매개변수

Subagent는 이러한 핵심 매개변수로 생성됩니다:

- **type**: Subagent 유형 (general-purpose, Explore, Plan, claude-code-guide, statusline-setup)
- **prompt**: Subagent를 위한 작업 또는 쿼리
- **run_in_background**: 비동기로 실행할지 여부 (기본값: false)
- **isolation**: 코드 수정 격리를 위한 선택적 git worktree 격리
- **parentAgentId**: 부모 Agent에서 추적용으로 설정됨
- **abortSignal**: 계층적 정리를 위한 선택적 취소 신호
- **workingDirectory**: 기본 작업 디렉토리 재정의
- **maxDuration**: 밀리초 단위의 시간 초과

### Agent 결과 구조

Subagent는 다음을 포함하는 결과 객체를 반환합니다:

- **agentId**: 이 Agent 실행의 고유 식별자
- **status**: 최종 상태 (completed, failed, 또는 aborted)
- **result**: Agent의 출력 또는 응답
- **error**: 상태가 failed인 경우 오류 세부 정보
- **duration**: 밀리초 단위의 실행 시간
- **worktreePath**: 격리가 사용되고 변경사항이 있는 경우 Worktree 경로
- **worktreeBranch**: Worktree가 생성된 경우 Git 브랜치 이름
