...
추출 알고리즘은 다중 단계 전략을 통해 노이즈를 제거하고 주요 콘텐츠를 우선시합니다:

1. **상용구 제거**: 스크립트, 스타일시트, 네비게이션, 푸터, 광고 컨테이너가 제거됩니다. 이는 노이즈를 줄이고 콘텐츠에 집중합니다.

2. **주요 콘텐츠 영역 식별**: 알고리즘은 `<article>`, `<main>` 또는 role="main"과 같은 의미론적 랜드마크를 스캔하여 주요 콘텐츠 블록을 찾습니다.

3. **의미 구조 보존**: 헤더, 목록, 코드 블록, 인용 및 강조는 마크다운 동등물로 변환됩니다 (예: `#`, `##`, `###`은 헤더 수준용, 코드는 백틱).

4. **형식 정규화**: 링크는 마크다운 구문 `[텍스트](url)`로 변환되고, 목록은 마크다운 글머리 또는 번호가 되며, 공백은 가독성을 위해 정규화됩니다.
...
탐지 시스템은 가져온 콘텐츠를 여러 패턴 카테고리에 대해 평가하여 인젝션 시도를 식별합니다:

1. **지시문 같은 텍스트**: 시스템 지시사항 (SYSTEM, RULE, INSTRUCTION, DIRECTIVE)을 스캔하여 모델 동작을 재정의하려는 시도를 찾습니다.

2. **역할 전환 시도**: 정체성 변경을 시도하는 언어 ("now you are", "pretend you", "act as")를 감지하여 모델을 잘못된 경로로 인도할 수 있습니다.

3. **시스템 프롬프트 모방**: 시스템 지시사항을 모방하거나 지시사항 준수를 해제하도록 요청하는 일반적인 사회 공학 기법인 텍스트를 식별합니다.

4. **도구 호출 하이재킹**: XML 같은 도구 사용 구문 또는 함수 호출 패턴을 찾아 모델이 의도하지 않은 작업을 호출하도록 속일 수 있습니다.

5. **이스케이프 시퀀스 및 숨겨진 콘텐츠**: HTML 주석의 숨겨진 콘텐츠 또는 정상 처리를 우회할 수 있는 의심스러운 유니코드 시퀀스를 식별합니다.

시스템은 **신뢰도 점수** (점수가 70%를 초과하면 플래그) 및 **상황 인식 필터링**을 사용하여 위양성을 줄입니다. 평가는 의심스러운 텍스트가 자연스러운 컨텍스트 (예: 문서에서 시스템 프롬프트 언급)에서 나타나는지 조작적인 컨텍스트에서 나타나는지 고려합니다.
